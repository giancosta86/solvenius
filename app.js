...
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.Title = Elm.Solvenius.Title || {};
Elm.Solvenius.Title.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.Title = _elm.Solvenius.Title || {};
   if (_elm.Solvenius.Title.values) return _elm.Solvenius.Title.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var ShowAbout = {ctor: "ShowAbout"};
   var ShowHelp = {ctor: "ShowHelp"};
   var ShowTopScore = {ctor: "ShowTopScore"};
   var NewGame = {ctor: "NewGame"};
   var view = function (commandAddress) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("titleStage")]),
      _U.list([A2($Html.h1,_U.list([$Html$Attributes.id("titleName")]),_U.list([$Html.text("Solvenius")]))
              ,A2($Html.h1,_U.list([$Html$Attributes.id("titleVersion")]),_U.list([$Html.text("3")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,commandAddress,NewGame)]),_U.list([$Html.text("New game")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,commandAddress,ShowTopScore)]),_U.list([$Html.text("Top score")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,commandAddress,ShowHelp)]),_U.list([$Html.text("Help")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,commandAddress,ShowAbout)]),_U.list([$Html.text("About")]))]));
   };
   return _elm.Solvenius.Title.values = {_op: _op,NewGame: NewGame,ShowTopScore: ShowTopScore,ShowHelp: ShowHelp,ShowAbout: ShowAbout,view: view};
};
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.GameRules = Elm.Solvenius.GameRules || {};
Elm.Solvenius.GameRules.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.GameRules = _elm.Solvenius.GameRules || {};
   if (_elm.Solvenius.GameRules.values) return _elm.Solvenius.GameRules.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var updateSecondsToPlay = F3(function (currentSecondsToPlay,correctSequence,correctSequencesOfCurrentLength) {
      var seriesLength = $Basics.toFloat(correctSequencesOfCurrentLength);
      var sequenceLength = $Basics.toFloat($List.length(correctSequence));
      var deltaSeconds = 5 * sequenceLength + sequenceLength / 5 * seriesLength;
      return currentSecondsToPlay + $Basics.ceiling(deltaSeconds);
   });
   var updateScore = F3(function (currentScore,correctSequence,elapsedSecondsForSequence) {
      var sequenceLength = $Basics.toFloat($List.length(correctSequence));
      var deltaScore = sequenceLength + 6 * sequenceLength / $Basics.toFloat(elapsedSecondsForSequence + 1);
      return currentScore + $Basics.ceiling(deltaScore);
   });
   var generateSequence = F2(function (length,seed) {
      var intToDigitChar = function (intSource) {    return $Char.fromCode($Char.toCode(_U.chr("0")) + intSource);};
      var digitGenerator = A2($Random.map,intToDigitChar,A2($Random.$int,0,9));
      var sequenceGenerator = A2($Random.list,length,digitGenerator);
      var _p0 = A2($Random.generate,sequenceGenerator,$Random.initialSeed(seed));
      var sequence = _p0._0;
      return sequence;
   });
   var assertValue = function (option) {
      var _p1 = option;
      if (_p1.ctor === "Just") {
            return _p1._0;
         } else {
            return _U.crashCase("Solvenius.GameRules",{start: {line: 60,column: 5},end: {line: 65,column: 65}},_p1)("Value expected but Nothing was found");
         }
   };
   var DigitWrong = {ctor: "DigitWrong"};
   var DigitMisplaced = {ctor: "DigitMisplaced"};
   var DigitOk = {ctor: "DigitOk"};
   var checkSequenceInternal = F4(function (inputList,secretList,fullSecretList,outputBuffer) {
      checkSequenceInternal: while (true) if (!_U.eq($List.length(inputList),$List.length(secretList))) return _U.crash("Solvenius.GameRules",
         {start: {line: 71,column: 9},end: {line: 71,column: 20}})(A2($Basics._op["++"],
         "inputList and secretList have different sizes! ",
         A2($Basics._op["++"],$Basics.toString($List.length(inputList)),A2($Basics._op["++"]," VS ",$Basics.toString($List.length(secretList))))));
      else if (_U.eq(inputList,_U.list([]))) return $List.reverse(outputBuffer); else {
               var secretTail = assertValue($List.tail(secretList));
               var secretHead = assertValue($List.head(secretList));
               var inputTail = assertValue($List.tail(inputList));
               var inputHead = assertValue($List.head(inputList));
               var digitResult = _U.eq(inputHead,secretHead) ? DigitOk : A2($List.member,inputHead,fullSecretList) ? DigitMisplaced : DigitWrong;
               var _v1 = inputTail,_v2 = secretTail,_v3 = fullSecretList,_v4 = A2($List._op["::"],digitResult,outputBuffer);
               inputList = _v1;
               secretList = _v2;
               fullSecretList = _v3;
               outputBuffer = _v4;
               continue checkSequenceInternal;
            }
   });
   var checkSequence = F2(function (inputSequence,secretSequence) {
      return A4(checkSequenceInternal,inputSequence,secretSequence,secretSequence,_U.list([]));
   });
   return _elm.Solvenius.GameRules.values = {_op: _op
                                            ,DigitOk: DigitOk
                                            ,DigitMisplaced: DigitMisplaced
                                            ,DigitWrong: DigitWrong
                                            ,assertValue: assertValue
                                            ,checkSequenceInternal: checkSequenceInternal
                                            ,checkSequence: checkSequence
                                            ,generateSequence: generateSequence
                                            ,updateScore: updateScore
                                            ,updateSecondsToPlay: updateSecondsToPlay};
};
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.Game = Elm.Solvenius.Game || {};
Elm.Solvenius.Game.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.Game = _elm.Solvenius.Game || {};
   if (_elm.Solvenius.Game.values) return _elm.Solvenius.Game.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Solvenius$GameRules = Elm.Solvenius.GameRules.make(_elm),
   $String = Elm.String.make(_elm),
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var renderDigitResult = function (_p0) {
      var _p1 = _p0;
      var cssDigitClass = function () {
         var _p2 = _p1._1;
         switch (_p2.ctor)
         {case "DigitOk": return "digitOk";
            case "DigitMisplaced": return "digitMisplaced";
            default: return "digitWrong";}
      }();
      return A2($Html.span,
      _U.list([$Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: cssDigitClass,_1: true},{ctor: "_Tuple2",_0: "outputDigit",_1: true}]))]),
      _U.list([$Html.text($String.fromChar(_p1._0))]));
   };
   var timeLabel = function (totalSeconds) {
      var seconds = A2($Basics._op["%"],totalSeconds,60);
      var minutes = totalSeconds / 60 | 0;
      return A2($Html.span,
      _U.list([]),
      _U.list([$Html.text(A2($Basics._op["++"],
      $Basics.toString(minutes),
      A2($Basics._op["++"]," min ",A2($Basics._op["++"],$Basics.toString(seconds)," s"))))]));
   };
   var unpackSequenceLogItem = function (sequenceLogItem) {
      var sequenceResult = $Basics.snd(sequenceLogItem);
      var sequence = $Basics.fst(sequenceLogItem);
      return A3($List.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),sequence,sequenceResult);
   };
   var renderSequenceLogItem = function (sequenceLogItem) {
      var digitResultPairs = unpackSequenceLogItem(sequenceLogItem);
      var digitResultOutputs = A2($List.map,renderDigitResult,digitResultPairs);
      return A2($Html.li,_U.list([]),digitResultOutputs);
   };
   var inGame = function (model) {    return _U.cmp(model.secondsToPlay,0) > 0;};
   var zeroDigitKeyCode = $Char.toCode(_U.chr("0"));
   var onAppendDigit = F2(function (digit,model) {
      if (_U.eq(model.secretSequence,_U.list([]))) return model; else {
            var elapsedSecondsForSequence = model.secondsToPlayAtLatestSuccess - model.secondsToPlay;
            var inputSequence = A2($Basics._op["++"],model.inputSequence,_U.list([digit]));
            var isInputSequenceComplete = _U.eq($List.length(inputSequence),model.secretSequenceLength);
            var isSequenceCorrect = isInputSequenceComplete && _U.eq(inputSequence,model.secretSequence);
            var score = isSequenceCorrect ? A3($Solvenius$GameRules.updateScore,model.score,model.secretSequence,elapsedSecondsForSequence) : model.score;
            var correctSequencesOfCurrentLength = isSequenceCorrect ? model.correctSequencesOfCurrentLength + 1 : model.correctSequencesOfCurrentLength;
            var mustIncreaseSequenceLength = _U.eq(correctSequencesOfCurrentLength,model.secretSequenceLength);
            var secondsToPlay = isSequenceCorrect ? A3($Solvenius$GameRules.updateSecondsToPlay,
            model.secondsToPlay,
            model.secretSequence,
            correctSequencesOfCurrentLength) : model.secondsToPlay;
            var secondsToPlayCap = isSequenceCorrect ? secondsToPlay : model.secondsToPlayCap;
            var secondsToPlayAtLatestSuccess = isSequenceCorrect ? secondsToPlay : model.secondsToPlayAtLatestSuccess;
            var sequenceLog = function () {
               if (isInputSequenceComplete) if (isSequenceCorrect) return _U.list([]); else {
                        var sequenceResult = A2($Solvenius$GameRules.checkSequence,inputSequence,model.secretSequence);
                        var sequenceLogItem = {ctor: "_Tuple2",_0: inputSequence,_1: sequenceResult};
                        return A2($List._op["::"],sequenceLogItem,model.sequenceLog);
                     } else return model.sequenceLog;
            }();
            return _U.update(model,
            {inputSequence: isInputSequenceComplete ? _U.list([]) : inputSequence
            ,secretSequence: isSequenceCorrect ? _U.list([]) : model.secretSequence
            ,score: score
            ,correctSequencesOfCurrentLength: mustIncreaseSequenceLength ? 0 : correctSequencesOfCurrentLength
            ,secretSequenceLength: mustIncreaseSequenceLength ? A2($Basics.min,model.secretSequenceLength + 1,9) : model.secretSequenceLength
            ,sequenceLog: sequenceLog
            ,secondsToPlay: secondsToPlay
            ,secondsToPlayCap: secondsToPlayCap
            ,secondsToPlayAtLatestSuccess: secondsToPlayAtLatestSuccess});
         }
   });
   var onTick = F2(function (time,model) {
      var secretSequence = _U.eq(model.secretSequence,_U.list([])) ? A2($Solvenius$GameRules.generateSequence,
      model.secretSequenceLength,
      $Basics.round(time * 3)) : model.secretSequence;
      var deltaFromLatestTickTime = time - model.latestTickTime;
      var isTickTime = _U.cmp(deltaFromLatestTickTime,1000) > -1;
      var secondsToPlay = isTickTime ? A2($Basics.max,model.secondsToPlay - 1,0) : model.secondsToPlay;
      var latestTickTime = isTickTime ? time : model.latestTickTime;
      return _U.update(model,{secondsToPlay: secondsToPlay,latestTickTime: latestTickTime,secretSequence: secretSequence});
   });
   var updateModel = F2(function (command,model) {
      if (inGame(model)) {
            var _p3 = command;
            switch (_p3.ctor)
            {case "TimeTick": return A2(onTick,_p3._0,model);
               case "AppendDigit": return A2(onAppendDigit,_p3._0,model);
               case "KeyPressed": var _p4 = _p3._0;
                 if (_U.cmp(zeroDigitKeyCode,_p4) < 1 && _U.cmp(_p4,zeroDigitKeyCode + 9) < 1) {
                       var digit = $Char.fromCode(_p4);
                       return A2(onAppendDigit,digit,model);
                    } else return model;
               default: return model;}
         } else return model;
   });
   var BackToTitle = function (a) {    return {ctor: "BackToTitle",_0: a};};
   var renderGameOver = F2(function (model,commandAddress) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("gameOverStage")]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Game over!")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("Your score is:")]))
              ,A2($Html.p,_U.list([$Html$Attributes.id("finalScoreLabel")]),_U.list([$Html.text($Basics.toString(model.score))]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.id("backButton"),A2($Html$Events.onClick,commandAddress,BackToTitle(model.score))]),
   ...
   var AppendDigit = function (a) {    return {ctor: "AppendDigit",_0: a};};
   var renderGame = F2(function (model,commandAddress) {
      var trailingString = A2($String.repeat,model.secretSequenceLength - $List.length(model.inputSequence),"_");
      var inputText = A2($Basics._op["++"],$String.fromList(model.inputSequence),trailingString);
      var makeNumberButton = function (digitValue) {
         var digitString = $Basics.toString(digitValue);
         var digit = $Char.fromCode(zeroDigitKeyCode + digitValue);
         return A2($Html.button,
         _U.list([$Html$Attributes.$class("digitButton"),A2($Html$Events.onClick,commandAddress,AppendDigit(digit))]),
         _U.list([$Html.text(digitString)]));
      };
      var topDigitButtons = A2($List.map,makeNumberButton,_U.range(1,5));
      var bottomDigitButtons = A2($List.map,makeNumberButton,A2($Basics._op["++"],_U.range(6,9),_U.list([0])));
      return A2($Html.div,
      _U.list([$Html$Attributes.id("gameStage")]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.id("scoreLabel")]),_U.list([$Html.text(A2($Basics._op["++"],"Score: ",$Basics.toString(model.score)))]))
              ,A2($Html.span,_U.list([$Html$Attributes.id("inputSequenceLabel")]),_U.list([$Html.text(inputText)]))
              ,A2($Html.ul,_U.list([$Html$Attributes.id("sequenceLog")]),A2($List.map,renderSequenceLogItem,model.sequenceLog))
              ,A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("digitButtonsRow")]),topDigitButtons)
                      ,A2($Html.div,_U.list([$Html$Attributes.$class("digitButtonsRow")]),bottomDigitButtons)]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.id("timeBox")]),
              _U.list([A2($Html.meter,
                      _U.list([$Html$Attributes.value($Basics.toString(model.secondsToPlay))
                              ,A2($Html$Attributes.attribute,"min","0")
                              ,A2($Html$Attributes.attribute,"low","10")
                              ,A2($Html$Attributes.attribute,"high","20")
                              ,A2($Html$Attributes.attribute,"optimum",$Basics.toString(A2($Basics.max,30,model.secondsToPlayCap)))
                              ,A2($Html$Attributes.attribute,"max",$Basics.toString(A2($Basics.max,30,model.secondsToPlayCap)))]),
                      _U.list([]))
                      ,timeLabel(model.secondsToPlay)]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.id("backButton"),A2($Html$Events.onClick,commandAddress,BackToTitle(model.score))]),
              _U.list([$Html.text("Leave game")]))]));
   });
   var view = F2(function (model,commandAddress) {    return inGame(model) ? A2(renderGame,model,commandAddress) : A2(renderGameOver,model,commandAddress);});
   var initModel = function () {
      var initialSecretSequenceLength = 2;
      var initialSecondsToPlay = 30;
      return {secondsToPlay: initialSecondsToPlay
             ,secondsToPlayCap: initialSecondsToPlay
             ,secondsToPlayAtLatestSuccess: initialSecondsToPlay
             ,score: 0
             ,latestTickTime: 0
             ,inputSequence: _U.list([])
             ,secretSequence: _U.list([])
             ,secretSequenceLength: initialSecretSequenceLength
             ,correctSequencesOfCurrentLength: 0
             ,sequenceLog: _U.list([])};
   }();
                  ...
                  return function (f) {
                     return function (g) {
                        return function (h) {
                           return function (i) {
                              return function (j) {
                                 return {secondsToPlay: a
                                        ,secondsToPlayCap: b
                                        ,secondsToPlayAtLatestSuccess: c
                                        ,score: d
                                        ,latestTickTime: e
                                        ,inputSequence: f
                                        ,secretSequence: g
                                        ,secretSequenceLength: h
                                        ,correctSequencesOfCurrentLength: i
                                        ,sequenceLog: j};
                              };
                           };
                        };
   ...
   return _elm.Solvenius.Game.values = {_op: _op
                                       ,Model: Model
                                       ,initModel: initModel
                                       ...
                                       ,zeroDigitKeyCode: zeroDigitKeyCode
                                       ,updateModel: updateModel
                                       ,inGame: inGame
                                       ,view: view
                                       ,renderGame: renderGame
                                       ,renderSequenceLogItem: renderSequenceLogItem
                                       ,unpackSequenceLogItem: unpackSequenceLogItem
                                       ,timeLabel: timeLabel
                                       ,renderDigitResult: renderDigitResult
                                       ,renderGameOver: renderGameOver};
};
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.TopScore = Elm.Solvenius.TopScore || {};
Elm.Solvenius.TopScore.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.TopScore = _elm.Solvenius.TopScore || {};
   if (_elm.Solvenius.TopScore.values) return _elm.Solvenius.TopScore.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Solvenius$GameRules = Elm.Solvenius.GameRules.make(_elm);
   var _op = {};
   var BackToTitle = {ctor: "BackToTitle"};
   var view = F2(function (model,commandAddress) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("topScoreStage")]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Top Score")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("Your top score up to now is:")]))
              ,A2($Html.p,_U.list([$Html$Attributes.id("topScoreLabel")]),_U.list([$Html.text($Basics.toString(model))]))
              ,A2($Html.button,_U.list([A2($Html$Attributes.attribute,"onclick","interactiveTopScoreReset()")]),_U.list([$Html.text("Reset")]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.id("backButton"),A2($Html$Events.onClick,commandAddress,BackToTitle)]),
              _U.list([$Html.text("Back")]))]));
   });
   return _elm.Solvenius.TopScore.values = {_op: _op,BackToTitle: BackToTitle,view: view};
};
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.HelpDocument = Elm.Solvenius.HelpDocument || {};
Elm.Solvenius.HelpDocument.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.HelpDocument = _elm.Solvenius.HelpDocument || {};
   if (_elm.Solvenius.HelpDocument.values) return _elm.Solvenius.HelpDocument.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
...
Elm.Solvenius.Help.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Solvenius$HelpDocument = Elm.Solvenius.HelpDocument.make(_elm);
   var _op = {};
   var BackToTitle = {ctor: "BackToTitle"};
   var view = function (commandAddress) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("helpStage")]),
      _U.list([$Solvenius$HelpDocument.helpContent
              ,A2($Html.button,
              _U.list([$Html$Attributes.id("backButton"),A2($Html$Events.onClick,commandAddress,BackToTitle)]),
              _U.list([$Html.text("Back")]))]));
   };
   return _elm.Solvenius.Help.values = {_op: _op,BackToTitle: BackToTitle,view: view};
};
Elm.Solvenius = Elm.Solvenius || {};
Elm.Solvenius.About = Elm.Solvenius.About || {};
Elm.Solvenius.About.make = function (_elm) {
   "use strict";
   _elm.Solvenius = _elm.Solvenius || {};
   _elm.Solvenius.About = _elm.Solvenius.About || {};
   if (_elm.Solvenius.About.values) return _elm.Solvenius.About.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var view = function (commandAddress) {
      ...
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Solvenius 3.4")]))
              ...
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("Copyright © 2001-2017 Gianluca Costa.")]))
              ,A2($Html.h3,_U.list([]),_U.list([$Html.text("Music credits")]))
              ...
              ,A2($Html.button,_U.list([A2($Html$Attributes.attribute,"onclick","openGithubPage()")]),_U.list([$Html.text("View on GitHub")]))
              ,A2($Html.button,_U.list([A2($Html$Attributes.attribute,"onclick","openFacebookPage()")]),_U.list([$Html.text("Solvenius on Facebook")]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.id("backButton"),A2($Html$Events.onClick,commandAddress,BackToTitle)]),
              _U.list([$Html.text("Back")]))]));
   };
   return _elm.Solvenius.About.values = {_op: _op,BackToTitle: BackToTitle,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Set = Elm.Set.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Solvenius$About = Elm.Solvenius.About.make(_elm),
   $Solvenius$Game = Elm.Solvenius.Game.make(_elm),
   $Solvenius$GameRules = Elm.Solvenius.GameRules.make(_elm),
   $Solvenius$Help = Elm.Solvenius.Help.make(_elm),
   $Solvenius$Title = Elm.Solvenius.Title.make(_elm),
   $Solvenius$TopScore = Elm.Solvenius.TopScore.make(_elm),
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var KeyPressed = function (a) {    return {ctor: "KeyPressed",_0: a};};
   var mapKeyPressed = function (keysDown) {    return KeyPressed(A2($Maybe.withDefault,0,$List.head($Set.toList(keysDown))));};
   var TopScoreInput = function (a) {    return {ctor: "TopScoreInput",_0: a};};
   var TimeTick = function (a) {    return {ctor: "TimeTick",_0: a};};
   var AboutCommand = function (a) {    return {ctor: "AboutCommand",_0: a};};
   var HelpCommand = function (a) {    return {ctor: "HelpCommand",_0: a};};
   var TopScoreCommand = function (a) {    return {ctor: "TopScoreCommand",_0: a};};
   var GameCommand = function (a) {    return {ctor: "GameCommand",_0: a};};
   var TitleCommand = function (a) {    return {ctor: "TitleCommand",_0: a};};
   var Nop = {ctor: "Nop"};
   var commandMailbox = $Signal.mailbox(Nop);
   var view = function (model) {
      var _p0 = model.state;
      switch (_p0.ctor)
      {case "InTitle": var titleAddress = A2($Signal.forwardTo,commandMailbox.address,function (titleCommand) {    return TitleCommand(titleCommand);});
           return $Solvenius$Title.view(titleAddress);
         case "InGame": var gameAddress = A2($Signal.forwardTo,commandMailbox.address,function (gameCommand) {    return GameCommand(gameCommand);});
           return A2($Solvenius$Game.view,_p0._0,gameAddress);
         case "InTopScore": var topScoreAddress = A2($Signal.forwardTo,
           commandMailbox.address,
           function (topScoreCommand) {
              return TopScoreCommand(topScoreCommand);
           });
           return A2($Solvenius$TopScore.view,model.topScore,topScoreAddress);
         case "InHelp": var helpAddress = A2($Signal.forwardTo,commandMailbox.address,function (helpCommand) {    return HelpCommand(helpCommand);});
           return $Solvenius$Help.view(helpAddress);
         default: var aboutAddress = A2($Signal.forwardTo,commandMailbox.address,function (aboutCommand) {    return AboutCommand(aboutCommand);});
           return $Solvenius$About.view(aboutAddress);}
   };
   var Model = F2(function (a,b) {    return {state: a,topScore: b};});
   var InAbout = {ctor: "InAbout"};
   var InHelp = {ctor: "InHelp"};
   var InTopScore = {ctor: "InTopScore"};
   var InGame = function (a) {    return {ctor: "InGame",_0: a};};
   var InTitle = {ctor: "InTitle"};
   var updateModel = F2(function (command,model) {
      var _p1 = command;
      if (_p1.ctor === "TopScoreInput") {
            return _U.update(model,{topScore: _p1._0});
         } else {
            var _p2 = model.state;
            switch (_p2.ctor)
            {case "InTitle": var _p3 = command;
                 if (_p3.ctor === "TitleCommand") {
                       var _p4 = _p3._0;
                       switch (_p4.ctor)
                       {case "NewGame": return _U.update(model,{state: InGame($Solvenius$Game.initModel)});
                          case "ShowTopScore": return _U.update(model,{state: InTopScore});
                          case "ShowHelp": return _U.update(model,{state: InHelp});
                          default: return _U.update(model,{state: InAbout});}
                    } else {
                       return model;
                    }
               case "InGame": var _p9 = _p2._0;
                 var _p5 = command;
                 switch (_p5.ctor)
                 {case "GameCommand": var _p8 = _p5._0;
                      var _p6 = _p8;
                      if (_p6.ctor === "BackToTitle") {
                            var _p7 = _p6._0;
                            return _U.update(model,{topScore: _U.cmp(_p7,model.topScore) > 0 ? _p7 : model.topScore,state: InTitle});
                         } else {
                            return _U.update(model,{state: InGame(A2($Solvenius$Game.updateModel,_p8,_p9))});
                         }
                    case "TimeTick": return _U.update(model,{state: InGame(A2($Solvenius$Game.updateModel,$Solvenius$Game.TimeTick(_p5._0),_p9))});
                    case "KeyPressed": return _U.update(model,{state: InGame(A2($Solvenius$Game.updateModel,$Solvenius$Game.KeyPressed(_p5._0),_p9))});
                    default: return model;}
               case "InTopScore": var _p10 = command;
                 if (_p10.ctor === "TopScoreCommand") {
                       return _U.update(model,{state: InTitle});
                    } else {
                       return model;
                    }
               case "InHelp": var _p11 = command;
                 if (_p11.ctor === "HelpCommand") {
                       return _U.update(model,{state: InTitle});
                    } else {
                       return model;
                    }
               default: var _p12 = command;
                 if (_p12.ctor === "AboutCommand") {
                       return _U.update(model,{state: InTitle});
                    } else {
                       return model;
                    }}
   ...
   var initModel = {state: InTitle,topScore: 0};
   var mapCommandToScoreOutput = function (command) {
      var _p13 = command;
      if (_p13.ctor === "GameCommand" && _p13._0.ctor === "BackToTitle") {
            return _p13._0._0;
         } else {
            return 0;
         }
   };
   var scoreOutput = Elm.Native.Port.make(_elm).outboundSignal("scoreOutput",
   function (v) {
      return v;
   },
   A2($Signal.map,mapCommandToScoreOutput,commandMailbox.signal));
   var topScoreInput = Elm.Native.Port.make(_elm).inboundSignal("topScoreInput",
   "Solvenius.TopScore.Model",
   function (v) {
      return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",v);
   });
   var command = $Signal.mergeMany(_U.list([A2($Signal.map,TopScoreInput,topScoreInput)
                                           ,commandMailbox.signal
                                           ,A2($Signal.map,mapKeyPressed,$Keyboard.keysDown)
                                           ,A2($Signal.map,TimeTick,$Time.every(50 * $Time.millisecond))]));
   var main = A2($Signal.map,view,A3($Signal.foldp,updateModel,initModel,command));
   return _elm.Main.values = {_op: _op
                             ,mapCommandToScoreOutput: mapCommandToScoreOutput
                             ,InTitle: InTitle
                             ,InGame: InGame
                             ,InTopScore: InTopScore
                             ,InHelp: InHelp
                             ,InAbout: InAbout
                             ,Model: Model
                             ,Nop: Nop
                             ...
                             ,TopScoreCommand: TopScoreCommand
                             ,HelpCommand: HelpCommand
                             ,AboutCommand: AboutCommand
                             ,TimeTick: TimeTick
                             ,TopScoreInput: TopScoreInput
                             ,KeyPressed: KeyPressed
                             ,commandMailbox: commandMailbox
                             ,command: command
                             ,mapKeyPressed: mapKeyPressed
                             ,updateModel: updateModel
                             ,view: view
                             ,initModel: initModel
                             ,main: main};
